...
def preprocess_user_input(user_input):
    ...
    inputWord1=nltk.tokenize(user_input)
    inputWord2=get_stem_words(inputWord1,ignore_words)
    inputWord3=sorted(list(set(inputWord2)))
    for word in words:
        if word in inputWord3:
   ...
   for intent in intents['intents']:
    if intent['tag']==predicted_class:
       ...
       # choose a random bot response
        bot_response = random.choice(intents["response"])
